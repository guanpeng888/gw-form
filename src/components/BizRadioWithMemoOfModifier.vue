<template>
  <div class="radio-list-wrap-style">
    <div class="radio-list-style" :style="'padding-right:'+(showSquare ? '20px' : '0')">
        ...
        >
          <span class="radio-item-style" v-for="(item , idx) in radioItems" :key="idx">
            <Radio v-if="!readonly" :label="idx" :true-value="selectedIndex === idx">
              <span class="labelText" @click="onRadioClick(idx)">{{item[bizViewCode]}} {{item[itemValueName]}}</span>
            </Radio>
            <span v-if="readonly" :class="['labelText', selectedIndex === idx ? 'active-labelText' : '']">{{item[bizViewCode]}} {{item[itemValueName]}}</span>
            <InputOfText
...
export default {
  name: "BizRadioWithMemoOfModifier",
  model: {
    prop: "initOpt",
    event: "change"
  },
  ...
  computed:{
    curSelectedCode(){
      // 当字典的bizViewCode属性存在则回显bizViewCode（因为bizViewCode用来显示，bizoptcode用来后端数据保存，所以两个code值可能存在不一样的问题），否则回显bizoptcode
      if(ObjectUtil.isNull(this.selectedIndex) && ObjectUtil.isNull(this.radioItems[this.selectedIndex][this.bizViewCode])){
  ...
  methods: {
    intiCtrl() {
      ...
      if (this.bizOptKey.bizoptcode && this.bizOptKey.bizoptname) {
        this.radioItems.map((item,index) => {
          ...
          if(Number(item[this.itemDefaultOpt])){
            ...
            radioDefaultOpt = JSON.parse(JSON.stringify(item));
          }
        });
      }

      // 默认值
      if(!this.radioSelectedItem[this.bizOptKey.bizoptcode] && !this.radioSelectedItem[this.bizOptKey.bizoptname] && this.selectedIndex !== null){
      ...
      if (this.radioSelectedItem) {
        for (let j = 0; j < this.radioItems.length; ++j) {
          if (
            String(this.radioSelectedItem[this.bizOptKey.bizoptcode]) === String(this.radioItems[j][this.bizOptKey.bizoptcode]) &&
    ...
    onRadioClick(idx){
      this.$nextTick(() => {
        this.radioSelectedItem = JSON.parse(JSON.stringify(this.radioItems[idx])) || {};
        this.$emit('onRadioClick',this.radioSelectedItem)
      })      
    },
    getInputValue(idx) {
      this.sendCtrlOpt(idx);
